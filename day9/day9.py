...
def insert_marble(marble_n, marbles):
    marbles.rotate(-1)
    marbles.append(marble_n)

def get_points(marble_n, marbles):
    marbles.rotate(7)
    score = marbles.pop()
    marbles.rotate(-1)

    return score + marble_n
...
def play_marbles(players, max_marble_n):
    ...
    for marble_n in range(1,max_marble_n+1):
        if marble_n % 23 == 0:
            score = get_points(marble_n, marbles)
            player_scores[current_player] += score
        else:
            insert_marble(marble_n, marbles)
        current_player = (current_player + 1) % players
    print(sorted([(player_scores[i],i) for i in player_scores])[-1])
